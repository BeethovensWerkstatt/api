...
declare function bw:getI18n($key as xs:string, $lang as xs:string, 
    ...
    let $langDoc := doc($config:app-root || '/resources/xql/tools/i18n.xml')
...
declare function bw:getMeasure($elem as node(), $lang as xs:string, $doc as node()) as xs:string {
    ...
    let $staff.ref :=
        if(not($staff.n))
        then()
        else if($doc//mei:staffDef[@n = $staff.n]/mei:label)
        then(concat(', ',($doc//mei:staffDef[@n = $staff.n and ./mei:label])[1]/mei:label/text()))
        else if($doc//mei:staffDef[@n = $staff.n]/ancestor::mei:staffGrp/mei:label)
        then(concat(', ',($doc//mei:staffDef[@n = $staff.n and ancestor::mei:staffGrp/mei:label])[1]/ancestor::mei:staffGrp[mei:label][1]/mei:label/text()))
        else()
...
:)
let $doc := $database/id($element.id)/ancestor::mei:mei
let $xslPath := $config:xslt-basepath || '../xsl/'
