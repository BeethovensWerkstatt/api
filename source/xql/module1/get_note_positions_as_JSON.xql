...
declare function vide:parsePath($array as xs:string*, $index as xs:integer, $ulx as xs:double, $uly as xs:double, $lrx as xs:double, $lry as xs:double, $current.x as xs:double, $current.y as xs:double) as element(dimensions) {
...
let $doc := collection($config:module1-root)//mei:mei[@xml:id = 'Codierung_op.75.2']
...
let $elems := 
    ...
    let $pages := 
        ...
        let $page.width := $surface/mei:graphic[@type = 'iiif']/xs:integer(@width)
        let $page.height := $surface/mei:graphic[@type = 'iiif']/xs:integer(@height)
