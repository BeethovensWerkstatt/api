...
let $shape.IDs := 
    if($elem/@facs) 
    then($elem/tokenize(normalize-space(replace(@facs,'#','')),' '))
    else if(local-name($elem) = 'note' and $elem/parent::mei:chord/@facs)
    then($elem/parent::mei:chord/tokenize(normalize-space(replace(@facs,'#','')),' '))
    else($elem//@facs/tokenize(normalize-space(replace(.,'#','')),' '))
let $shapes :=
    for $shape.id in $shape.IDs
    let $shape := collection($config:module1-root)//svg:path[@id = $shape.id]
    where not($doc//mei:zone[@xml:id = $shape.id])
    return $shape
let $svg.ids := distinct-values($shapes/ancestor::svg:svg[last()]/string(@id))

let $pages := 
    for $svg.id in $svg.ids
    let $surface := $doc//mei:surface[child::mei:graphic[@target = $svg.id and @type = 'shapes']]
    let $page.label := if($surface/@label) then($surface/string(@label)) else($surface/string(@n))
    let $source.label := 
    ...
    let $page.uri := $surface/mei:graphic[@type = 'iiif']/string(@target)
    let $page.width := $surface/mei:graphic[@type = 'iiif']/number(@width)
    let $page.height := $surface/mei:graphic[@type = 'iiif']/number(@height)
    ...
    let $limited.by := if($snippet.ratio lt $target.ratio) then('h') else('w')
    let $factor := max(($target.ratio,$snippet.ratio)) div min(($target.ratio,$snippet.ratio))
    
    let $raw.w := $lrx - $ulx
    let $raw.h := $lry - $uly
    
    let $snippet.x := ($ulx - $raw.w * .5)
    let $snippet.y := ($uly - $raw.h * .5)
    let $snippet.x2 := ($lrx + $raw.w * .5)
    let $snippet.y2 := ($lry + $raw.h * .5)
