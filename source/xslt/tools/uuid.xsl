...
-->
    ...
    <xsl:function name="uuid:get-uuid" as="xs:string*">
        ...
        <xsl:value-of separator="-" select="
            substring($ts, 8, 8),
            substring($ts, 4, 4),
            string-join((uuid:get-uuid-version(),
            substring($ts, 1, 3)), ''),
    ...
    <xsl:function name="uuid:next-nr" as="xs:integer">
        <xsl:variable name="node">
            <xsl:sequence select="uuid:_get-node()"/>
        </xsl:variable>
        <xsl:sequence select="
            xs:integer(replace(
            generate-id($node), '\D', ''))"/>
    ...
    <xsl:function name="uuid:get-network-node" as="xs:string">
        <xsl:sequence select="uuid:_hex-only('09-27-3F-13-E4-C5', 12)"/>
    </xsl:function>
