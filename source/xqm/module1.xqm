...
module namespace module1 = "https://beethovens-werkstatt.de/module1";
...
declare function module1:getPosition($states as node()+, $follows as xs:string*, $prev as xs:string*, $position as xs:integer, $scar.ordered as xs:boolean) as xs:integer {
    ...
    let $positions :=
        ...
        let $state.position := module1:getPosition($states, $state.follows, $state.prev, $position + 1, $scar.ordered)
...
declare function module1:getStatesJson($scar as node(), $enriched.doc as node()) as xs:string* {
    ...
    let $position := module1:getPosition($scar/mei:state, $follows, $prev, 1, $scar.ordered)
...
declare function module1:getElementsWithinScar($doc as node(), $scar.id as xs:string) as node()* {
...
declare function module1:getEnrichedFile($doc as node()) as node() {
    let $xslPath := '../xsl/' 
